...
def find_next(h, w, puzzle_table, puzzles, puzzle_put_count, ans):
    """迭代找拼圖解"""
    ...
    for offset_h in range(h-puzzles[puzzle_put_count][0]+1):
        for offset_w in range(w-puzzles[puzzle_put_count][1]+1):
            ...
            # print(puzzles)
            next_p = False
            puzzle_table = old_puzzle_table

            # 檢查棋盤, 在該位置上能不能放下拼圖
            for puzzle in puzzles[puzzle_put_count][2]:
                if puzzle_table[(puzzle[0]+offset_h, puzzle[1]+offset_w)] == 1:
                    next_p = True
                    break
            ...
            # 放拼圖下去
            for puzzle in puzzles[puzzle_put_count][2]:
                puzzle_table[(puzzle[0]+offset_h, puzzle[1]+offset_w)] = 1
            ...
            # 下一個迭帶
            find_next(h, w, puzzle_table, puzzles, puzzle_put_count+1, ans)

            # 還原拼圖
            for puzzle in puzzles[puzzle_put_count][2]:
                puzzle_table[(puzzle[0]+offset_h, puzzle[1]+offset_w)] = 0
...
if __name__ == '__main__':
    ...
    h = input_num('輸入高: ')
    w = input_num('輸入寬: ')
    puzzle_table = np.zeros((h, w), dtype=int)
    ...
    ans = []
    # 計算答案
    find_next(h, w, puzzle_table, puzzles, 0, ans)
